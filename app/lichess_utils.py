...
def _get_active_items(t: dict) -> list:
    """Return list of active item types for a task node."""
    items = []
    if t.get('puzzles', 0) > 0: items.append("puzzles")
    if t.get('controls', 0) > 0: items.append("controls")
    if t.get('exams', 0) > 0: items.append("exams")
    return items


def _clean_stepchess_title(title: Optional[str]) -> str:
    """Strip English translations and extra whitespace from Stepchess titles."""
    if not title:
        return ""
    return title.split("|")[0].strip()


def _format_stepchess_line(course: str, parent_title: str, task_title: str, item_type: str, t: dict) -> Text:
    c_clean = _clean_stepchess_title(course)
    p_clean = _clean_stepchess_title(parent_title)
    t_clean = _clean_stepchess_title(task_title)
    
    if item_type == "controls":
        t_clean = t_clean or "Контрольная работа"
    elif item_type == "exams":
        t_clean = t_clean or "Экзамен"
    elif item_type == "puzzles":
        t_clean = t_clean or "Задачи"
        
    parts = [p for p in [c_clean, p_clean, t_clean] if p]
    header = ". ".join(parts)
    ...
    if item_type == "puzzles":
        ...
        time_info = f" | ⏱️ {total_time_str} (ср. {avg_time_str})" if t['time'] > 0 else ""
        stats = f"{t['puzzles']} зад. (✅ {t.get('puzzles_success', 0)}/{t['puzzles']}) | 🔄 {p_attempts} поп. (ср. {p_avg}){time_info}"
        ...
        count_zad = t['controls'] * 10
        stats = f"{count_zad} зад. (✅ {t.get('controls_success', 0)}/{t['controls']}) | 🔄 {c_attempts} поп. (ср. {c_avg}){solved_str}"
    else:  # exams
        e_attempts = t.get('exams_attempts', 0)
        e_avg = round(e_attempts / t['exams'], 1) if t['exams'] > 0 else 0
        stats = f"{t['exams']} экз. (✅ {t.get('exams_success', 0)}/{t['exams']}) | 🔄 {e_attempts} поп. (ср. {e_avg})"
        
    return Text("🧩 ", Bold(header), f": {stats}")
        ...
        if "stepchess" in s["stats"]:
            for course, c in s["stats"]["stepchess"]["breakdown"].items():
                if "parents" in c and c["parents"]:
                    ...
                    for parent_title, p_data in parents_list:
                        for task_title, t in p_data["tasks"].items():
                            for item_type in _get_active_items(t):
                                report_items.append(_format_stepchess_line(course, parent_title, task_title, item_type, t))
                else:
                    for item_type in _get_active_items(c):
                        report_items.append(_format_stepchess_line(course, "", "", item_type, c))
        report_items.append("")
    return as_list(*report_items).as_html()
...
def generate_rank_leaderboard(students: List[Student]) -> str:
    ranks_order = [
        "КМС",
        "1 разряд",
        "2 разряд",
    ...
    report_items = [Text("🏅 ", Bold("Рейтинг по разрядам:")), ""]
    global_idx = 1
    
    for r in ranks_order:
        filtered = [s for s in students if s.rank == r]
        if not filtered:
            continue
...
async def sync_annual_tournament_results(team_id: str, students: List[Student], full_rebuild: bool = False) -> List[tuple]:
    ...
    """
    # Try Firestore cache first (max 1 hour old)
    all_tournaments = None
    try:
        doc = await db.collection("metadata").document("team_tournaments").get()
        if doc.exists:
            data = doc.to_dict()
            cached_at_str = data.get("updated_at")
            if cached_at_str:
                cache_age = (datetime.utcnow() - datetime.fromisoformat(cached_at_str)).total_seconds()
                if cache_age < 3600:
                    all_tournaments = data.get("tournaments", [])
                    logging.info("sync_annual: loaded team tournaments from Firestore cache.")
    except Exception as e:
        logging.error(f"Error reading team tournaments from Firestore cache: {e}")
